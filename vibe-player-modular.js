...
import AdaptiveQualityManager from './src/core/adaptive-quality.js';
import QualityProfiles from './src/core/quality-profiles.js';
import ParticlePool from './src/utils/particle-pool.js';
import VectorPool from './src/utils/vector-pool.js';
...
class VibePlayer {
  constructor() {
    ...
    // Performance settings - will be overridden by quality manager
    this.settings = QualityProfiles.high;
    
    // Initialize performance systems
    this.qualityManager = new AdaptiveQualityManager(this);
    this.performanceMonitor = this.qualityManager.getMonitor();
    ...
    // Initialize quality system after everything is set up
  ...
  drawVisualization() {
    ...
    if (this.nextVisualization && this.transitionProgress < 1) {
      ...
      this.drawVisualizationType(this.currentVisualization, dataArray, width, height, this.settings);
      this.ctx.restore();
      
      // Draw next visualization with fade in
      this.ctx.save();
      this.ctx.globalAlpha = this.transitionProgress;
      this.drawVisualizationType(this.nextVisualization, dataArray, width, height, this.settings);
      this.ctx.restore();
    ...
    this.ctx.fillText(`FPS: ${Math.round(metrics.fps)} | Quality: ${this.qualityManager.currentProfile}`, 10, 10);
    this.ctx.restore();
  }
  
  drawVisualizationType(type, dataArray, width, height, settings) {
    switch (type) {
      case 'bars':
        this.drawBars(dataArray, width, height, settings);
        break;
      case 'wave':
        this.drawWave(dataArray, width, height, settings);
        break;
      case 'circle':
        this.drawCircle(dataArray, width, height, settings);
        break;
      case 'particles':
        this.drawParticles(dataArray, width, height, settings);
        break;
      case 'galaxy':
        this.drawGalaxy(dataArray, width, height, settings);
        break;
      case 'dna':
        this.drawDNA(dataArray, width, height, settings);
        break;
      case 'ribbons':
        this.drawRibbons(dataArray, width, height, settings);
        break;
      case 'fractal':
        this.drawFractal(dataArray, width, height, settings);
        break;
      case 'neon':
        this.drawNeonGrid(dataArray, width, height, settings);
        break;
    }
  }
  
  drawBars(dataArray, width, height, settings) {
    const barCount = settings.frequencyBands || 32;
